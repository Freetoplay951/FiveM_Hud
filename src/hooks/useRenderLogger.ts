        ...
        if (changes.length > 0 || Object.keys(trackedValues).length === 0) {
            ...
            if (groupLogs && changes.length > 1) {
                console.groupCollapsed(
                    `%c[Render] %c${componentName} %c#${renderCount} %c(${changes.length} changes)${isFrequent ? ' ⚠️ FREQUENT' : ''}`,
...
export function logRenderSummary(): void {
    ...
    const stats = Array.from(renderCounts.entries())
        .sort((a, b) => b[1] - a[1]);
...
if (typeof window !== 'undefined' && RENDER_LOGGING_ENABLED) {
    (window as unknown as { __renderStats: { get: () => Record<string, number>; reset: () => void; log: () => void } }).__renderStats = {
        get: getRenderStats,
