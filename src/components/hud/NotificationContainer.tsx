...
export const NotificationContainer = ({ notifications, onClose, isWidget = false }: NotificationContainerProps) => {
    ...
    const orderedNotifications = [...notifications].reverse().slice(0, MAX_VISIBLE_NOTIFICATIONS * 2);
            ...
            <AnimatePresence mode="popLayout">
                {orderedNotifications.map((notification, index) => {
                    ...
                    const stackOffset = isStacked
                        ? MAX_VISIBLE_NOTIFICATIONS * (NOTIFICATION_HEIGHT + NOTIFICATION_GAP) +
                          (index - MAX_VISIBLE_NOTIFICATIONS) * 4
                        : yOffset;
